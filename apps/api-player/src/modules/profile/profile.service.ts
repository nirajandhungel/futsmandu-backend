...
export class ProfileService {
  ...
  async confirmAvatarUpload(userId: string, assetId: string, key: string) {
    ...
    // Derive the CDN URL from the key using the configured CDN base or endpoint.
    const cdnBase = ENV['S3_CDN_BASE_URL'] || ENV['S3_ENDPOINT'] || ''
    const baseUrl = cdnBase.endsWith('/') ? cdnBase.slice(0, -1) : cdnBase
    const cdnUrl = baseUrl ? `${baseUrl}/${key}` : null
